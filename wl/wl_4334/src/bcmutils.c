 ...
 * $Id: bcmutils.c 313209 2012-02-07 01:30:18Z $
 */
...
#if !defined(BCMDONGLEHOST) && defined(SAVERESTORE)
#include <hndsrmem.h>
#endif /* SAVERESTORE */
...
/* ROM tunable:
 *   Global (4 bytes RAM) is less than ROM accessor func (16 bytes ROM + 4 bytes RAM)
 */
uint16 g_pktpool_maxlen = PKTPOOL_LEN_MAX;
uint16 g_pktpool_sizeof = sizeof(pktpool_t);
...
BCMATTACHFN(pktpool_init)(osl_t *osh, pktpool_t *pktp, int *pplen, int plen, bool istx)
{
	...
	bzero(pktp, g_pktpool_sizeof);
	pktp->inited = TRUE;
	pktp->istx = istx ? TRUE : FALSE;
	pktp->plen = (uint16)plen;
	*pplen = 0;

	pktp->maxlen = g_pktpool_maxlen;
...
char *
bcm_ipv6_ntoa(struct ipv6_addr *ia, char *buf)
{
	snprintf(buf, 40, "%x%02x:%x%02x:%x%02x:%x%02x:%x%02x:%x%02x:%x%02x:%x%02x",
		ia->addr[0], ia->addr[1], ia->addr[2], ia->addr[3],
		ia->addr[4], ia->addr[5], ia->addr[6], ia->addr[7],
		ia->addr[8], ia->addr[9], ia->addr[10], ia->addr[11],
		ia->addr[12], ia->addr[13], ia->addr[14], ia->addr[15]);
	return (buf);
}
...
static int
BCMATTACHFN(getintvar_internal)(char *vars, const char *name)
{
	char *val;
...
static int
BCMATTACHFN(getintvararray_internal)(char *vars, const char *name, int index)
{
	char *buf, *endp;
	int i = 0;
	int val = 0;
...
static int
BCMATTACHFN(getintvararraysize_internal)(char *vars, const char *name)
{
	char *buf, *endp;
	int count = 0;
	int val = 0;
...
pktsetprio(void *pkt, bool update_vtag)
{
	...
	ASSERT(ISALIGNED(pktdata, sizeof(uint16)));
